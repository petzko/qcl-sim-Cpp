...
protected:
	...
	// as every matrix M can be the parent of more than 1 views, for memory management issues the
	// parent matrix needs to keep track of all "active views" on itself. This is needed, for instance, if
	// parent matrix is freed. In such cases all correspondingly registered views have to be freed as well
	// since they will refer to invalid memory location.
...
public:
	...
	friend Matrix<_Tp> operator*(_Tp lhs, const Matrix<_Tp>& rhs) {
		...
		switch (type) {
		case FLT:
			cblas_sscal(N * M, explicit_cast(float,lhs), (float*) *res._data,
					1);
			break;
		case DBL:
			cblas_dscal(N * M, explicit_cast(double,lhs), (double*) *res._data,
					1);
			break;
		case CPLXFLT:
			cblas_cscal(N * M, (void*) &lhs, (void*) (*res._data), 1);
			break;
		case CPLXDBL:
			cblas_zscal(N * M, (void*) &lhs, (void*) (*res._data), 1);
			break;
		default:
